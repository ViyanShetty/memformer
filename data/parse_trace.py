...
def parse_trace(filepath, warmup=0, use=200_000, min_freq=2):
    pcs, addrs = [], []
    with open(filepath) as f:
        for i, line in enumerate(f):
            ...
            try:
                pcs.append(int(parts[0], 16) & 0xFFFFFFFF)
                addrs.append(int(parts[1], 16) & 0xFFFFFFFF)
            except:
                continue
    ...
    freq  = Counter(deltas.tolist())
    valid = {d for d, c in freq.items() if c >= min_freq}
    mask  = np.array([d in valid for d in deltas.tolist()], dtype=bool)
    deltas = deltas[mask]
    pcs    = pcs[mask]

    vocab = sorted(valid)
    d2i   = {d: i for i, d in enumerate(vocab)}
    delta_ids = np.array([d2i[d] for d in deltas.tolist()])
